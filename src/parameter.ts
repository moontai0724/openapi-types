 ...
 *   Templating](https://spec.openapis.org/oas/latest.html#path-templating),
 *   where the parameter value is actually part of the operation’s URL. This
 ...
 * [location](https://spec.openapis.org/oas/latest.html#parameterIn).
 *
...
export interface BaseParameterObject extends Extendable {
   ...
   *
   * - If [`in`](https://spec.openapis.org/oas/latest.html#parameterIn) is
   *   `"path"`, the `name` field _MUST_ correspond to a template expression
   *   occurring within the
   *   [path](https://spec.openapis.org/oas/latest.html#pathsPath) field in the
   ...
   *   the [`in`](https://spec.openapis.org/oas/latest.html#parameterIn)
   *   property.
   */
  name: string;
  /**
   ...
   * location](https://spec.openapis.org/oas/latest.html#parameterIn) is
   * `"path"`, this property is **_REQUIRED_** and its value _MUST_ be `true`.
   ...
   * [`style`](https://spec.openapis.org/oas/latest.html#parameterStyle) is
   * used, and if behavior is `n/a` (cannot be serialized), the value of
   ...
   * of the parameter value. Default values (based on value of `in`): for
   ...
   * only applies to parameters with an `in` value of `query`. The default value
   * is `false`.
   */
  ...
  schema?: SchemaObject;
  /**
   * Example of the parameter’s potential value. The example _SHOULD_ match the
   ...
   * `schema` that contains an example, the `example` value _SHALL_ _override_
   * the example provided by the schema. To represent examples of media types
   * that cannot naturally be represented in JSON or YAML, a string value can
   * contain the example with escaping where necessary.
   */
  example?: any;
  /**
   * Examples of the parameter’s potential value. Each example _SHOULD_ contain
   ...
   * _SHALL_ _override_ the example provided by the schema.
   */
 ...
 * [`schema`](https://spec.openapis.org/oas/latest.html#parameterSchema) and
 * [`style`](https://spec.openapis.org/oas/latest.html#parameterStyle) can
 * describe the structure and syntax of the parameter. A parameter _MUST_
...
/**
 * The rules for serialization of the parameter are specified in one of two
 * ways. For more complex scenarios, the
 * [`content`](https://spec.openapis.org/oas/latest.html#parameterContent)
 * property can define the media type and schema of the parameter. A parameter
 * _MUST_ contain either a `schema` property, or a `content` property, but not
 * both. When `example` or `examples` are provided in conjunction with the
 * `schema` object, the example _MUST_ follow the prescribed serialization
